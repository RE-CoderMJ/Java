...
public class EasyRun {
	...
	public static void main(String[] args) {
		...
		// 추상클래스로 객체 생성 불가 (왜? 미완성 클래스기 때문에!!)
		//Sports s = new Sports();
		//Sports s; // 객체 생성이 완될뿐이지 레퍼런스로는 사용 가능
		Sports s = new BasketBall(); // 다형성 적용시켜서 부모타입으로 자식객체를 받아주는 형태 가능
		 ...
		 * 			  런타임시점(실행시점)에는 동적바인딩으로 각각의 자식클래스에 오버라이딩 된 메소드가 있을 경우 알아서 찾아져서 실행됨
		 */
		 ...
		 * 	 => 각 자식 클래스마다 실행시킬 내용은 다르지만 동일한 형태의 메소드로 구현했으면 할때
		 * 		=> 메소드 통일성 확보
		 * 		=> 표준화된 틀을 제공할 목적
		 */
